...
object AccountStorage {
    ...
    fun saveAccount(
        ...
        newDeviceAlertsEnabled: Boolean = true,
        publicWifiWarningEnabled: Boolean = false
    ) {
    ...
    fun loadAccount(context: Context): StoredAccount? {
        ...
        val appLockEnabled = prefs.getBoolean(KEY_APP_LOCK_ENABLED, true) // Default True
        ...
        val publicWifiWarningEnabled = prefs.getBoolean(KEY_WIFI_WARNING, false)

        return StoredAccount(
            name, email, password, biometricEnabled,
            isDarkTheme, appLockEnabled,
            loginAlertsEnabled, newDeviceAlertsEnabled, publicWifiWarningEnabled
        )
    }
